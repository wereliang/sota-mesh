...
var (
	...
	// ErrTooManyConcurrency is returned when concurrency is over the cb maxConcurrency
	ErrTooManyConcurrency = errors.New("too many concurrency")
)
...
func (c *Concurrency) In() {
	atomic.AddInt32(&c.cnt, 1)
}

func (c *Concurrency) Out() {
	atomic.AddInt32(&c.cnt, -1)
}
...
func (c *Concurrency) Access() bool {
	if c.max <= 0 {
		return true
	}
	return atomic.LoadInt32(&c.cnt) <= c.max
}
...
func NewCircuitBreaker(st Settings) *CircuitBreaker {
	...
	cb.concurrency = &Concurrency{max: int32(st.MaxConcurrency)}
	return cb
}
...
func (cb *CircuitBreaker) Concurrency() *Concurrency {
...
func (cb *CircuitBreaker) Execute(req func() (interface{}, error)) (interface{}, error) {
	// concurrency check
	if !cb.concurrency.Access() {
		return nil, ErrTooManyConcurrency
	}
	cb.concurrency.In()
	defer cb.concurrency.Out()
